...
namespace zhaiwei
{
    ...
    void unweighted_undirected_dfs_visit(const graph &g, dfs_tree &tree, int root_idx, vertex_color colors[], int &time)
    {
        ...
        for (iter = g.m_headers[root_idx]->begin(); iter != g.m_headers[root_idx]->end(); ++iter)
        {
            ...
            if (colors[No-1] == gray)
            {
                pair<dfs_tree::edge::iterator, dfs_tree::edge::iterator> range;
                dfs_tree::edge::iterator eiter;
                range = tree.m_tree_edges.equal_range(name);
                for (eiter = range.first; eiter != range.second; ++eiter)
                    if (root_name == eiter->second)
                        break;
                if (eiter == range.second)
                    tree.m_back_edges.insert(make_pair(root_name, name));
            }
            else if (colors[No-1] == white)
            {
                tree.m_tree_edges.insert(make_pair(root_name, name));
                unweighted_undirected_dfs_visit(g, tree, No-1, colors, time);
    ...
    void unweighted_undirected_dfs(const graph &g, dfs_forest &forest)
    {
        ...
        for (size_t i = 0; i != g.m_headers.size(); ++i)
        {
            ...
            {
                unweighted_undirected_dfs_visit(g, tree, i, colors, time);
